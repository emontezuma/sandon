  ...
  calcularTiempo()
{
  ...
  contarRegs()
  {
    ...
    let sentencia = "SELECT SUM(a1) AS alarmados, SUM(a2) AS total FROM (SELECT SUM(IF((alarmado_atender = 'S' AND estatus = 0) OR (alarmado_atendido = 'S' AND estatus = 10), 1, 0)) AS a1, COUNT(*) AS a2 FROM " + this.servicio.rBD() + ".reportes WHERE (estatus = 0 OR estatus = 10) UNION SELECT SUM(IF(alarmado = 'S', 1, 0)) AS a1, COUNT(*) AS a2 FROM " + this.servicio.rBD() + ".kanban_solicitudes WHERE estado < 40) AS q1";
    let campos = {accion: 100, sentencia: sentencia};  
    this.servicio.consultasBD(campos).subscribe( resp =>
    {
      let mensaje = "";
