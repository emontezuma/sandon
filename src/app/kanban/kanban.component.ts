...
export class KanbanComponent implements OnInit {
  ...
  rConfiguracion()
  {
    ...
    this.servicio.consultasBD(campos).subscribe( resp =>
    {
      ...
      {
        this.cAreaSurtir = "";
        sentencia = "SELECT nombre FROM " + this.servicio.rBD() + ".cat_areas WHERE id = " + +resp[0].kanban_area;
        campos = {accion: 100, sentencia: sentencia};  
...
//      vistaRecuadro = this.servicio.rUsuario().preferencias_andon.substr(this.miSeleccion - 1, 1) == "1";
  ...
  totales()
  {
    ...
    let sentencia = "SELECT SUM(IF(estado <= 10, 1, 0)) AS kp, SUM(IF(estado >= 40, 1, 0)) AS kc, SUM(IF(alarmado = 'S' AND estado <= 10, 1, 0)) AS kpa FROM " + this.servicio.rBD() + ".kanban_solicitudes " + cadFiltro;
