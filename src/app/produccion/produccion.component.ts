      ...
      respuesta.afterClosed().subscribe(result => {
        if (result.accion)
        {
          if (result.accion == 1) 
          {
            ...
            let sentencia = "UPDATE " + this.servicio.rBD() + ".cat_maquinas SET oee_parte_actual = " + this.detalle.parteActual + 
...
sesion(opcion: number)
{
  ...
  {
    this.detalle.parteActual = this.parteActual;
    let sentencia = "SELECT id, IF(ISNULL(referencia), nombre, CONCAT(nombre, '" + this.servicio.rTraduccion()[2637] + "', referencia, ')')) AS nombre FROM " + this.servicio.rBD() + ".cat_partes WHERE oee = 'S' AND estatus = 'A' AND tipo = 0 ORDER BY nombre;"
    this.partes = [];
    let campos = {accion: 100, sentencia: sentencia};  
    this.servicio.consultasBD(campos).subscribe( resp =>
    {
  ...
  {
    this.detalle.loteActual = this.loteActual
    let sentencia = "SELECT a.id, IF(ISNULL(b.nombre), a.numero, CONCAT(a.numero, ' (Nro Parte: ', b.nombre, ')')) AS numero FROM " + this.servicio.rBD() + ".lotes a LEFT JOIN " + this.servicio.rBD() + ".cat_partes b ON a.parte = b.id WHERE a.estatus = 'A' AND a.estado <= 50 ORDER BY numero;"
    let campos = {accion: 100, sentencia: sentencia};  
    this.servicio.consultasBD(campos).subscribe( resp =>
    {
      this.lotes = resp;
  ...
  buscarDatos(indice: number, evento: any)
  {
    ...
    else if (indice == 2)
    {
      ...
      //Buscar parte
      this.servicio.activarSpinnerSmall.emit(true);
      let sentencia = "SELECT a.id, b.id AS parte, IF(ISNULL(b.nombre), a.numero, CONCAT(a.numero, ' (Nro Parte: ', b.nombre, ')')) AS numero FROM " + this.servicio.rBD() + ".lotes a LEFT JOIN " + this.servicio.rBD() + ".cat_partes b ON a.parte = b.id WHERE a.id = " + idBuscar;
