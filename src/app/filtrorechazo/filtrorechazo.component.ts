...
export class FiltrorechazoComponent implements OnInit {
  ...
  carrusel()
 {
    let sentencia = "SELECT a.id, IF(ISNULL(b.nombre), a.nombre, CONCAT(a.nombre, ' / ',  b.nombre)) AS nombre FROM " + this.servicio.rBD() + ".cat_maquinas a LEFT JOIN " + this.servicio.rBD() + ".cat_lineas b ON a.linea = b.id ORDER BY nombre"
