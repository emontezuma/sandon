...
export class AndonComponent implements OnInit {
  ...
  rRegistros(tabla: number)
  {
    ...
    else if (tabla == 2)
    {
      ...
      sentencia = "SELECT a.id, a.nombre, IFNULL(a.url_mmcall, '" + this.servicio.rTraduccion()[8] + "') AS url_mmcall, a.imagen, IFNULL(c.nombre, '" + this.servicio.rTraduccion()[8] + "') AS nlinea, 'S' AS mostrarImagen FROM " + this.servicio.rBD() + ".cat_maquinas a INNER JOIN " + this.servicio.rBD() + ".relacion_usuarios_operaciones b ON a.id = b.proceso AND b.tipo = 2 AND b.usuario = " + this.servicio.rUsuario().id + " LEFT JOIN " + this.servicio.rBD() + ".cat_lineas c ON a.linea = c.id WHERE a.estatus = 'A' AND (a.linea = " + +this.laSeleccion[0] + " OR a.linea = 0) ORDER BY a.nombre;"
      if (this.servicio.rUsuario().maquina=="S")
      {
        sentencia = "SELECT a.id, a.nombre, IFNULL(a.url_mmcall, '" + this.servicio.rTraduccion()[8] + "') AS url_mmcall, a.imagen, IFNULL(c.nombre, '" + this.servicio.rTraduccion()[8] + "') AS nlinea, 'S' AS mostrarImagen FROM " + this.servicio.rBD() + ".cat_maquinas a LEFT JOIN " + this.servicio.rBD() + ".cat_lineas c ON a.linea = c.id WHERE a.estatus = 'A' AND (a.linea = " + +this.laSeleccion[0] + " OR a.linea = 0) ORDER BY a.nombre;"
      }      
    ...
    else if (tabla == 3)
    {
      ...
      sentencia = "SELECT a.id, a.nombre, IFNULL(a.url_mmcall, '" + this.servicio.rTraduccion()[8] + "') AS url_mmcall, a.imagen, 'S'  AS mostrarImagen FROM " + this.servicio.rBD() + ".cat_areas a INNER JOIN " + this.servicio.rBD() + ".relacion_usuarios_operaciones b ON a.id = b.proceso AND b.tipo = 3 AND b.usuario = " + this.servicio.rUsuario().id + " WHERE a.estatus = 'A' AND a.andon = 'S' " + cadAdic + " ORDER BY a.nombre;"
      if (this.servicio.rUsuario().area=="S")
      {
        sentencia = "SELECT a.id, a.nombre, IFNULL(a.url_mmcall, '" + this.servicio.rTraduccion()[8] + "') AS url_mmcall, a.imagen, 'S'  AS mostrarImagen FROM " + this.servicio.rBD() + ".cat_areas a WHERE a.estatus = 'A' AND a.andon = 'S' " + cadAdic + " ORDER BY a.nombre;"
      }
    ...
    else if (tabla == 11 || tabla == 12 || tabla == 13 || tabla == 14 || tabla == 15 )
    {
      ...
      else if (tabla == 14 || tabla == 15)
      {
        ...
        sentencia = "SELECT a.id, a.estatus, SEC_TO_TIME(a.tiemporeparacion + a.tiempollegada) AS tiempo1, a.alarmado_atender, a.alarmado_atendido, a.inicio_atencion, a.fecha, IF(a.estatus = 0, a.fecha, IF(a.estatus = 10, a.inicio_atencion, a.inicio_reporte)) AS fechac, IFNULL(b.nombre, '" + this.servicio.rTraduccion()[8] + "') AS nlinea, IFNULL(c.nombre, '" + this.servicio.rTraduccion()[8] + "') AS nmaquina, IFNULL(d.nombre, '" + this.servicio.rTraduccion()[8] + "') AS narea, IFNULL(e.nombre, '" + this.servicio.rTraduccion()[8] + "') AS nfalla, IFNULL(f.nombre, '" + this.servicio.rTraduccion()[8] + "') AS nsolicitante, a.origen FROM " + this.servicio.rBD() + ".reportes a LEFT JOIN " + this.servicio.rBD() + ".cat_lineas b ON a.linea = b.id LEFT JOIN " + this.servicio.rBD() + ".cat_maquinas c ON a.maquina = c.id LEFT JOIN " + this.servicio.rBD() + ".cat_areas d ON a.area = d.id LEFT JOIN " + this.servicio.rBD() + ".cat_fallas e ON a.falla = e.id LEFT JOIN " + this.servicio.rBD() + ".cat_usuarios f ON a.solicitante = f.id WHERE a.estatus <= 100 " + filtroLinea + " " + filtroMaquina + " " + filtroArea + " ORDER BY a.fecha " + (tabla == 14 ? " ASC" : " DESC ");
  ...
  {
    this.herramentales = [];
    let sentencia = "SELECT a.id, IF(ISNULL(a.referencia), a.nombre, CONCAT(a.nombre, '" + this.servicio.rTraduccion()[2637] + "', a.referencia, ')')) AS nombre FROM " + this.servicio.rBD() + ".cat_partes a WHERE herramentales = 'S' AND maquinas = 'S' OR id IN (SELECT parte FROM " + this.servicio.rBD() + ".relacion_partes_maquinas WHERE maquina = " + this.detalle.maquina + ") ORDER BY 2"
    let campos = {accion: 100, sentencia: sentencia};  
    this.servicio.consultasBD(campos).subscribe( resp =>
    {
  ...
  leerBD()
  {
    ...
    this.servicio.consultasBD(campos).subscribe( resp =>
    {
      ...
      if (this.router.url.substr(0, 6) == "/andon"  && this.enTecnico)
      {
  ...
  reparar(id: number)
  {
    ...
    respuesta.afterClosed().subscribe(result => 
    {
      if (result)
      {
        if (result.accion == 1) 
        {
          let sentencia = "UPDATE " + this.servicio.rBD() + ".reportes SET tecnicoatend = " + +this.servicio.rUsuario().id + ", estatus = 10, inicio_atencion = IF(NOW() < fecha, DATE_ADD(fecha, INTERVAL 5 SECOND), NOW()), tiempollegada = TIME_TO_SEC(TIMEDIFF(NOW(), fecha)) WHERE id = " + this.reporteSeleccionado;
          let campos = {accion: 200, sentencia: sentencia};  
          this.servicio.consultasBD(campos).subscribe( resp =>
          {
            let mensajeCompleto: any = [];
  ...
  totales()
  {
    ...
    let sentencia = "SELECT SUM(IF(estatus <= 100, 1, 0)) AS rst, SUM(IF(estatus = 0, 1, 0)) AS rsa, SUM(IF(alarmado_atender  = 'S' AND estatus = 0, 1, 0)) AS rsaa, SUM(IF(estatus = 10, 1, 0)) AS rer, SUM(IF(alarmado_atendido = 'S' AND estatus = 10, 1, 0)) AS rera, SUM(IF(estatus = 100, 1, 0)) AS ral, SUM(IF(alarmado = 'S' AND estatus = 100, 1, 0)) AS rala FROM " + this.servicio.rBD() + ".reportes " + cadFiltro;
  ...
  prevalidar(vez: number)
  {
    ...
    else if (vez == 3)
    {
      ...
      if (this.servicio.rUsuario().area=="S")
      {
        sentencia = "SELECT a.id, a.nombre, IFNULL(a.url_mmcall, '" + this.servicio.rTraduccion()[8] + "') AS url_mmcall, a.imagen, 'S'  AS mostrarImagen FROM " + this.servicio.rBD() + ".cat_areas a WHERE a.estatus = 'A' " + cadAdic + " ORDER BY a.nombre;"
      }
    }
    else if (vez == 4)
    {
