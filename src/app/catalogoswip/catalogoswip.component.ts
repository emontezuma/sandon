...
export class CatalogoswipComponent implements OnInit {
  ...
  recuperar()
  {
    ...
    else if (this.maestroActual == 0 && this.nivelActual == 2)
    {
      ...
      this.llenarAreasK();
      sentencia = "SELECT a.*, IF(ISNULL(b.referencia), b.nombre, CONCAT(b.nombre, '" + this.servicio.rTraduccion()[2637] + "', b.referencia, ')')) AS parte, a.parte AS herramental FROM " + this.servicio.rBD() + ".relacion_operaciones_partes a INNER JOIN " + this.servicio.rBD() + ".cat_partes b ON a.parte = b.id WHERE a.id = " + this.registroActual; 
      this.arreHover4.length = 2;
    }
    else if (this.maestroActual == 1 && this.nivelActual == 0)
    {
