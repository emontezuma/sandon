...
export class DialogoComponent implements OnInit {
  ...
  listarRutas()
  {
    ...
    sentencia = "SELECT a.id, IF(ISNULL(a.referencia), a.nombre, CONCAT(a.nombre, '" + this.servicio.rTraduccion()[2637] + "', a.referencia, ')')) AS nombre, a.nombre AS nnombre, b.url_mmcall AS dunidad FROM " + this.servicio.rBD() + ".cat_partes a LEFT JOIN " + this.servicio.rBD() + ".cat_generales b ON a.kanban_unidad = b.id WHERE kanban = 'S' ORDER BY 2"
    campos = {accion: 100, sentencia: sentencia};  
    this.servicio.consultasBD(campos).subscribe( resp =>
    {
      resp.splice(0, 0, {id: 0, nombre: this.servicio.rTraduccion()[1493]});
  ...
  buscarDatos(indice: number, evento: any)
  {
    if (indice == 1)
    {
      ...
      //Buscar parte
      this.servicio.activarSpinnerSmall.emit(true);
      let sentencia = "SELECT a.id, IF(ISNULL(a.referencia), a.nombre, CONCAT(a.nombre, '" + this.servicio.rTraduccion()[2637] + "', a.referencia, ')')) AS nombre, a.nombre AS nnombre, b.url_mmcall AS dunidad FROM " + this.servicio.rBD() + ".cat_partes a LEFT JOIN " + this.servicio.rBD() + ".cat_generales b ON a.kanban_unidad = b.id WHERE a.id = " + idBuscar;
      let campos = {accion: 100, sentencia: sentencia};  
      this.servicio.consultasBD(campos).subscribe( resp =>
      {
        if (resp.length > 0)
